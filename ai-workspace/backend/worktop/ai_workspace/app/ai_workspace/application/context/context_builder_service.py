...
from worktop.ai_workspace.app.ai_workspace.domain.chat_message import ChatMessage
...
class ContextBuilderService:
    ...
    def __init__(self, file_read_service: FileReadService, session_service: SessionService, governance: DataGovernanceService | None = None):
        self._file_read_service = file_read_service
        self._session_service = session_service
        self._governance = governance or DataGovernanceService()

    def build(self, runtime: WorkspaceRuntime) -> ContextBundle:
        files = [item for path in runtime.selected_file_paths if (item := self._read_context_file(runtime.workspace_path, path)) is not None]
        messages = self._session_service.get_messages(runtime.session_id)[-RECENT_MESSAGE_WINDOW:]
        return ContextBundle(workspace_path=runtime.workspace_path, files=files, recent_messages=messages, blocked_files=list(self._governance.audit.files_blocked), redaction_count=self._governance.audit.redactions)

    def _read_context_file(self, workspace_path: str, relative_path: str) -> ContextFile | None:
        repo_file = self._file_read_service.read(workspace_path, relative_path)
        content = self._governance.release(relative_path, repo_file.content)
        if content is None:
            return None
