...
from worktop.ai_workspace.app.ai_workspace.domain.chat_message import ChatMessage, ChatRole
from worktop.ai_workspace.app.ai_workspace.domain.execution_context import (
    ExecutionContext,
    ExecutionStage,
    ExecutionStageStatus,
    ExecutionStatus,
)
from worktop.ai_workspace.app.ai_workspace.domain.execution_plan import ExecutionPlan, ExecutionPlanStep, PlannedToolCall
from worktop.ai_workspace.app.ai_workspace.domain.file_change import DiffHunk, DiffLine, FileChange, FileChangeStatus
...
from worktop.ai_workspace.app.ai_workspace.domain.workspace_session import WorkspaceSession
from worktop.ai_workspace.app.ai_workspace.infrastructure.state_store import StateStore
...
def _execution_to_dict(execution: ExecutionContext) -> dict:
    return {
        ...
        "review_reasons": execution.review_reasons,
        "budget_usage": execution.budget_usage,
        "engineering_review": execution.engineering_review,
        "isolated_workspace_path": execution.isolated_workspace_path,
    }
...
def _execution_from_dict(payload: dict) -> ExecutionContext:
    return ExecutionContext(
        ...
        engineering_review=dict(payload.get("engineering_review", {})),
        isolated_workspace_path=payload.get("isolated_workspace_path"),
    )
...
def _file_change_to_dict(change: FileChange) -> dict:
    return {
        ...
        "original_existed": change.original_existed,
    }
...
def _file_change_from_dict(payload: dict) -> FileChange:
    return FileChange(
        ...
        original_existed=bool(payload.get("original_existed", False)),
    )
