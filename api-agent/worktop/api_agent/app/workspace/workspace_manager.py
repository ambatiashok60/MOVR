...
class WorkspaceManager:
    ...
    def __init__(self, workspace_root: str | None = None) -> None:
        self.root = Path(workspace_root or settings.workspace_root) / "api-agent-workspaces"
    ...
    def acquire(self, job_id: str, repo_path: str) -> JobWorkspace:
        ...
        logger.info(
            "Workspace acquired for job %s on %s (%s).",
            job_id, repo_path, workspace_dir,
        )
        return workspace
    ...
    def release(self, workspace: JobWorkspace) -> None:
        try:
            ...
            if holder.get("job_id") == workspace.job_id:
                ...
                logger.info(
                    "Workspace released for job %s on %s.",
                    workspace.job_id, workspace.repo_path,
                )
        except OSError as exc:
            logger.warning(
                "Workspace release failed for job %s: %s", workspace.job_id, exc
            )

    def snapshot_targets(self, workspace: JobWorkspace, relative_paths: list[str]) -> None:
        """Copy the pre-write content of every target file into the workspace."""
        root = Path(workspace.repo_path)
        for relative_path in sorted(set(relative_paths)):
            source = root / relative_path
            if not source.is_file():
                continue
            destination = workspace.snapshot_dir / relative_path
    ...
    def record_files(self, workspace: JobWorkspace, files: list[GeneratedFile]) -> None:
        for file in files:
            self._append(
    ...
    def record_rollback(self, workspace: JobWorkspace, paths: list[str]) -> None:
        for path in paths:
            self._append(
                workspace.rollback_journal,
                {"job_id": workspace.job_id, "path": path, "at": self._now()},
            )
        ...
        while True:
            try:
                ...
                if acquired and time.time() - acquired > self.stale_lock_seconds:
                    logger.warning(
                        "Stale repository lock reclaimed by job %s (stale holder: %s).",
                        job_id, holder.get("job_id", "unknown"),
                    )
